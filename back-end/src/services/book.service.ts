...
export interface IBookService {
    ...
    findBook(filterOptions: Map<string, unknown>): Promise<RequestSuccess<OutPutBook[]>>
}
...
export class BookService implements IBookService {
    async getBookById(bookId: string): Promise<RequestSuccess<OutPutBook>> {
        ...
        return new RequestSuccess(HttpCode.OK, result, `Retrieving book with ID ${bookId}`)
    }
    ...
    async findBook(filterOptions: Map<string, unknown>): Promise<RequestSuccess<OutPutBook[]>> {
        const options = Utils.cleanFilterOptions(filterOptions)
        const books = await Book.find()
        const collections = await BookCollection.find({ $or: options })
        const result = (await this.bindCollection(books, collections)).sort((a, b) =>
    ...
    async findBookByISBN(isbn: string): Promise<RequestSuccess<OutPutBook[]>> {
        ...
        return new RequestSuccess(HttpCode.OK, result, `Finding book with ISBN like ${isbn}`)
    }

    private async updateCollectionQuantity(collectionId: string, quantity: number) {
    ...
    private async bindCollection(
        ...
        books.forEach((element) => {
            const collection = collections.filter((value) => value._id.toString() == element.collectionId)[0]
            result.push({
                collection,
                book: element,
            })
