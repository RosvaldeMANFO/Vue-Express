...
export class RequestService implements IRequestService {
  ...
  async getAllRequest(): Promise<RequestSuccess<IRequest[]>> {
    ...
    result = result.filter((request) =>
      acceptedStatus.includes(request.requestStatus)
    );
    return new RequestSuccess(HttpCode.OK, result, "Getting all request");
  }

  async getRequestByIdOrEmail(
    query: string
  ): Promise<RequestSuccess<IRequest[]>> {
  ...
  private async updateStock(
    ...
    if (
      borrowStatus == RequestStatus.Approved &&
      borrow?.requestStatus != RequestStatus.Approved
    ) {
    ...
    if (
      borrowStatus == RequestStatus.Returned &&
      borrow?.requestStatus != RequestStatus.Returned
    ) {
      await BookCollection.findByIdAndUpdate(stock!.id, {
        quantity: stock!.quantity + 1,
      });
